 ...
 * The caller is responsible for calling free() on @output if *output is non-NULL.
 */
int copy_offload_copy(COPY_OFFLOAD *offload, char *compute_name, char *workflow_name, int dry_run, const char *profile_name, char *source_path, char *dest_path, char **output) {
    long http_code;
    struct memory chunk = {NULL, 0};
    int ret = 1;
    char urlbuf[COPY_OFFLOAD_URL_SIZE];
    ...
    char *offload_req =
        ...
        "\"logStdout\": true, "
        "\"storeStdout\": false}";
    snprintf(postbuf, COPY_OFFLOAD_POST_SIZE, offload_req, compute_name, workflow_name, source_path, dest_path, profile_name, dry_run_str);
