...
func checkIndexMountDir(clnt client.Client, ctx context.Context, dm *nnfv1alpha11.NnfDataMovement) (string, error) {
	...
	if dm.Spec.Source.StorageReference.Kind == reflect.TypeOf(nnfv1alpha11.NnfStorage{}).Name() {
		...
		nodeStorage = &nnfv1alpha11.NnfNodeStorage{}
...
func mpiStat(profile *nnfv1alpha11.NnfDataMovementProfile, path string, uid, gid uint32, mpiHostfile string, log logr.Logger) (string, error) {
...
func mpiIsDir(profile *nnfv1alpha11.NnfDataMovementProfile, path string, uid, gid uint32, mpiHostfile string, log logr.Logger) (bool, error) {
...
func isSourceAFile(profile *nnfv1alpha11.NnfDataMovementProfile, dm *nnfv1alpha11.NnfDataMovement, mpiHostFile string, log logr.Logger) (bool, error) {
	var isDir bool
	var err error
...
// on that.
func isDestAFile(profile *nnfv1alpha11.NnfDataMovementProfile, dm *nnfv1alpha11.NnfDataMovement, mpiHostFile string, log logr.Logger) bool {
	isFile := false
	exists := true
...
func createDestinationDir(profile *nnfv1alpha11.NnfDataMovementProfile, dm *nnfv1alpha11.NnfDataMovement, dest, mpiHostfile string, log logr.Logger) error {
...
func GetWorkerHostnames(clnt client.Client, ctx context.Context, nodes []string) ([]string, error) {
	...
	listOptions := []client.ListOption{
		client.InNamespace(nnfv1alpha11.DataMovementNamespace),
		client.MatchingLabels(map[string]string{
	...
	for _, pod := range pods.Items {
		...
		nodeNameToHostnameMap[pod.Spec.NodeName] = fmt.Sprintf(
			...
			nnfv1alpha11.DataMovementNamespace)
	}
