...
func ParseDcpProgress(line string, cmdStatus *nnfv1alpha10.NnfDataMovementCommandStatus) error {
...
func ParseDcpStats(line string, cmdStatus *nnfv1alpha10.NnfDataMovementCommandStatus) error {
...
func GetDMProfile(clnt client.Client, ctx context.Context, dm *nnfv1alpha10.NnfDataMovement) (*nnfv1alpha10.NnfDataMovementProfile, error) {

	var profile *nnfv1alpha10.NnfDataMovementProfile

	if dm.Spec.ProfileReference.Kind != reflect.TypeOf(nnfv1alpha10.NnfDataMovementProfile{}).Name() {
...
func BuildDMCommand(profile *nnfv1alpha10.NnfDataMovementProfile, hostfile string, usePermissions bool, dm *nnfv1alpha10.NnfDataMovement, log logr.Logger) ([]string, error) {
...
func PrepareDestination(clnt client.Client, ctx context.Context, profile *nnfv1alpha10.NnfDataMovementProfile, dm *nnfv1alpha10.NnfDataMovement, mpiHostfile string, log logr.Logger) error {
...
func checkIndexMountDir(clnt client.Client, ctx context.Context, dm *nnfv1alpha10.NnfDataMovement) (string, error) {
	var storage *nnfv1alpha10.NnfStorage
	var nodeStorage *nnfv1alpha10.NnfNodeStorage

	if dm.Spec.Source.StorageReference.Kind == reflect.TypeOf(nnfv1alpha10.NnfStorage{}).Name() {
	...
	} else if dm.Spec.Source.StorageReference.Kind == reflect.TypeOf(nnfv1alpha10.NnfNodeStorage{}).Name() {
		// The source storage reference is NnfNodeStorage - this came from copy_offload
		storageRef := dm.Spec.Source.StorageReference

		nodeStorage = &nnfv1alpha10.NnfNodeStorage{}
...
func HandleIndexMountDir(profile *nnfv1alpha10.NnfDataMovementProfile, dm *nnfv1alpha10.NnfDataMovement, destDir, indexMount, mpiHostfile string, log logr.Logger) (string, error) {
...
func GetDestinationDir(profile *nnfv1alpha10.NnfDataMovementProfile, dm *nnfv1alpha10.NnfDataMovement, mpiHostfile string, log logr.Logger) (string, error) {
...
func mpiStat(profile *nnfv1alpha10.NnfDataMovementProfile, path string, uid, gid uint32, mpiHostfile string, log logr.Logger) (string, error) {
...
func mpiIsDir(profile *nnfv1alpha10.NnfDataMovementProfile, path string, uid, gid uint32, mpiHostfile string, log logr.Logger) (bool, error) {
...
func isSourceAFile(profile *nnfv1alpha10.NnfDataMovementProfile, dm *nnfv1alpha10.NnfDataMovement, mpiHostFile string, log logr.Logger) (bool, error) {
	var isDir bool
	var err error
...
// on that.
func isDestAFile(profile *nnfv1alpha10.NnfDataMovementProfile, dm *nnfv1alpha10.NnfDataMovement, mpiHostFile string, log logr.Logger) bool {
	isFile := false
	exists := true
...
func createDestinationDir(profile *nnfv1alpha10.NnfDataMovementProfile, dm *nnfv1alpha10.NnfDataMovement, dest, mpiHostfile string, log logr.Logger) error {
...
func CreateMpiHostfile(profile *nnfv1alpha10.NnfDataMovementProfile, hosts []string, dm *nnfv1alpha10.NnfDataMovement) (string, error) {
	userConfig := dm.Spec.UserConfig != nil
...
func GetStorageNodeNames(clnt client.Client, ctx context.Context, dm *nnfv1alpha10.NnfDataMovement) ([]string, error) {

	// If this is a node data movement request simply reference the localhost
	if dm.Namespace == os.Getenv("NNF_NODE_NAME") || isTestEnv() || dm.Spec.Source.StorageReference.Kind == reflect.TypeOf(nnfv1alpha10.NnfNodeStorage{}).Name() {
	...
	if dm.Spec.Source.StorageReference.Kind == reflect.TypeOf(nnfv1alpha10.NnfStorage{}).Name() {
		storageRef = dm.Spec.Source.StorageReference
	} else if dm.Spec.Destination.StorageReference.Kind == reflect.TypeOf(nnfv1alpha10.NnfStorage{}).Name() {
...
func GetCopyOffloadWorkerHostnames(clnt client.Client, ctx context.Context, nodes []string, workflow, namespace string, dm *nnfv1alpha10.NnfDataMovement) ([]string, error) {
	...
	if nodes[0] == "localhost" {
		...
		if dm.Spec.Source.StorageReference.Kind != reflect.TypeOf(nnfv1alpha10.NnfNodeStorage{}).Name() {
...
func GetWorkerHostnames(clnt client.Client, ctx context.Context, nodes []string) ([]string, error) {
	...
	listOptions := []client.ListOption{
		client.InNamespace(nnfv1alpha10.DataMovementNamespace),
		client.MatchingLabels(map[string]string{
			nnfv1alpha10.DataMovementWorkerLabel: "true",
		}),
	}
	pods := &corev1.PodList{}
	if err := clnt.List(ctx, pods, listOptions...); err != nil {
	...
	for _, pod := range pods.Items {
		...
		nodeNameToHostnameMap[pod.Spec.NodeName] = fmt.Sprintf(
			...
			nnfv1alpha10.DataMovementNamespace)
	}
