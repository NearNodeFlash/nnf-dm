...
func (r *DataMovementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	defer func() {
		if err != nil {
			resourceError, ok := err.(*dwsv1alpha6.ResourceErrorInfo)
			if ok {
				if resourceError.Severity != dwsv1alpha6.SeverityMinor {
					dm.Status.State = nnfv1alpha8.DataMovementConditionTypeFinished
					dm.Status.Status = nnfv1alpha8.DataMovementConditionReasonInvalid
				}
			}
	...
	if dm.Spec.Cancel {
		if err := r.cancel(ctx, dm); err != nil {
			return ctrl.Result{}, dwsv1alpha6.NewResourceError("").WithError(err).WithUserMessage("Unable to cancel data movement")
		}
	...
	if dm.Status.State == nnfv1alpha8.DataMovementConditionTypeRunning {
	...
	if err != nil {
		return ctrl.Result{}, dwsv1alpha6.NewResourceError("could not get profile for data movement").WithError(err).WithMajor()
	}
	log.Info("Using profile", "profile", profile)

	nodes, err := GetStorageNodeNames(r.Client, ctx, dm)
	if err != nil {
		return ctrl.Result{}, dwsv1alpha6.NewResourceError("could not get storage nodes for data movement").WithError(err).WithMajor()
	}

	hosts, err := GetWorkerHostnames(r.Client, ctx, nodes)
	if err != nil {
		return ctrl.Result{}, dwsv1alpha6.NewResourceError("could not get worker nodes for data movement").WithError(err).WithMajor()
	}
	...
	if err != nil {
		return ctrl.Result{}, dwsv1alpha6.NewResourceError("could not create MPI hostfile").WithError(err).WithMajor()
	}
	...
	if err != nil {
		return ctrl.Result{}, dwsv1alpha6.NewResourceError("could not create data movement command").WithError(err).WithMajor()
	}
...
func (r *DataMovementReconciler) cancel(ctx context.Context, dm *nnfv1alpha8.NnfDataMovement) error {
	log := log.FromContext(ctx)
...
func (r *DataMovementReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&nnfv1alpha8.NnfDataMovement{}).
