...
alias dmpod="kubectl describe pod `dmget` -n nnf-dm-system"
...
alias wrklog="kubectl logs `workerget` -n nnf-dm-system"

alias lchrget="kubectl get pods -n nnf-dm-system --no-headers | grep mpi-launcher | head -n1 | awk '{print \$1}'"
alias lchryaml="kubectl get pod/`launcherpod` -n nnf-dm-system -o yaml"
alias lchrlog="kubectl logs pod/`launcherpod` -n nnf-dm-system"
alias lchrdes="kubectl describe pod/`launcherpod` -n nnf-dm-system"
alias lchrsh="kubectl exec --stdin --tty `launcherpod` -n nnf-dm-system -- /bin/bash"
